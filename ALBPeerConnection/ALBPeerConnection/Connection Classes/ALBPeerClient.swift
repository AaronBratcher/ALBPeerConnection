...
protocol ALBPeerClientDelegate {
	...
	:param: errorDict A dictionary containing details of the error.
	*/
	...
	:param: server An ALBPeer object initialized with a name, unique identifier, and other information used by the class.
	*/
	...
	:param: server An ALBPeer object initialized with a name and unique identifier.
	*/
...
class ALBPeerClient:NSObject,NSNetServiceBrowserDelegate,NSNetServiceDelegate,GCDAsyncSocketDelegate {
	...
	**/
	...
	:param: server An ALBPeer object returned by the serverFound delegate call.
	*/
	func connectToServer(server:ALBPeer) {
        ...
        if let socket = GCDAsyncSocket(delegate: self, delegateQueue: ALBPeerConnectionQueue, socketQueue: ALBPeerConnectionQueue), hostName = server.service?.hostName, port = server.service?.port {
            ...
            var error:NSError?
            connecting = socket.connectToHost(hostName, onPort: UInt16(port), error: &error)
            if connecting {
                _socket = socket
                _server = server
            }
	...
	*/
    func netService(service: NSNetService, didUpdateTXTRecordData data: NSData) {
        if let txtDict = NSNetService.dictionaryFromTXTRecordData(data) as? [NSObject:NSObject], idObject = txtDict["peerID"] as? NSData, peerID =  NSString(data: idObject, encoding: NSUTF8StringEncoding) as? String {
            if nearbyServers.filter({$0.peerID == peerID}).count == 0 {
                var remoteNode = ALBPeer(name: service.name, peerID: peerID)
